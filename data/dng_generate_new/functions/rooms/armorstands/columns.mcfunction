...
execute if score $zcolumns_size_opperate dng_generate matches 1 run scoreboard players operation $ylevels_size_opperate dng_generate = $levels dng_generate
...
execute if score $zcolumns_size_opperate dng_generate matches 1.. as @e[type=armor_stand,tag=dng_newLine,tag=dng_line] at @s positioned ~ ~ ~35 run summon armor_stand ~ ~ ~ {NoGravity:1b,Tags:["dng_as","dng_gen_as","dng_newLine","dng_newLevel"],Team:"dng_red",Glowing:1b}
...
execute if score $zcolumns_size_opperate dng_generate matches 0 run schedule function dng_generate_new:rooms/armorstands/levels 1t